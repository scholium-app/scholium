...
/// 本机 CJK 字体路径。缺失时应用仍启动，但界面会报告字体缺口。
const CJK_FONT_PATH: &str = "/usr/share/fonts/adobe-source-han-serif/SourceHanSerifCN-Regular.otf";

/// 本机 CJK 字体族名，供 iced 的 `Font::with_name` 使用。
pub const CJK_FAMILY: &str = "Source Han Serif CN";
...
impl App {
    /// 建立应用。`cjk_font` 表示是否已成功加载 CJK 字体。
    pub fn boot(cjk_font: bool) -> Self {
...
impl App {
    ...
    /// 当前输入焦点所在区域。
    pub fn input_area(&self) -> InputArea {
        self.input_area
    }
