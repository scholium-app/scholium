...
- `Item::Text` 的纵坐标是**基线**，不是绘制框上沿。渲染方必须用 `Item::top_of(baseline, size)`
  换算，不要各自猜字体度量——早期版本因为各方按"上沿"摆放，上下标相对基线偏了一整行。
- `ASCENT_RATIO` 是唯一的近似常数，所有图元共用；同字号精确对齐，跨字号误差与字号差成正比。
- 上下标偏移用排版惯例（上标 −0.42em、下标 +0.20em），有测试断言防止漂移。
- 核对布局数字用 `cargo run --example dump_layout`，不要对着截图猜。
