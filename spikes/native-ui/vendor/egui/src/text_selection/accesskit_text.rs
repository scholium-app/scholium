...
pub fn update_accesskit_for_text_widget(
    ...
    // Keep row geometry local so scrolling changes one transform, rather than
    // sending a bounds update for every line to the platform accessibility bus.
    let container_id = widget_id.with("text_geometry");
    ctx.register_accesskit_parent(container_id, parent_id);
    ctx.accesskit_node_builder(container_id, |builder| {
        builder.set_role(accesskit::Role::GenericContainer);
        builder.set_transform(accesskit::Affine::new([
            global_from_galley.scaling.into(),
            0.0,
            0.0,
            global_from_galley.scaling.into(),
            global_from_galley.translation.x.into(),
            global_from_galley.translation.y.into(),
        ]));
    });
    ...
    for (row_index, row) in galley.rows.iter().enumerate() {
        ...
        if total_chars <= MAX_CHARS_PER_TEXT_RUN {
            ...
            ctx.register_accesskit_parent(run_id, container_id);
            ...
            ctx.accesskit_node_builder(run_id, |builder| {
                ...
                let rect = row.rect_without_leading_space();
