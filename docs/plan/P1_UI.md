...
| 层级 | 执行方式 |
|---|---|
| 必须保持的原则 | 科学排版定位；所见即所得是单一编辑面；源码与预览同等重要；文件操作归属文件菜单；正文优先；权威模型及构建状态不含混 |
| 默认起点 | 源码与预览约 1:1；文件导航和诊断默认收起；Ribbon 按功能标签与分组组织；菜单、快捷键、功能区调用同一命令 |
...
下列两图来自早期工作区草图，保存在仓库内，脱离会话也可阅读。它们仅说明单一编辑面与源码分屏的关系，
顶部菜单和工具栏以本页的 Ribbon 组织为准；没有展示的下方正文、状态栏或菜单内容以文字描述为准。
不作为产品截图、固定主题或像素验收样本。
...
默认收起文件导航和诊断；页面视口使用 Ribbon 以下、状态栏以上的全部空间。
...
LaTeX 发布目标的最终检查由“布局 → 排版 → 最终排版检查”按需打开，不占据日常所见即所得布局。
...
| Ribbon 标签栏 | 文档标签下方，左侧为文件菜单与开始、插入、布局、视图；右侧为帮助和折叠开关。折叠后标签仍可访问 |
| Ribbon 命令区 | 只显示当前标签的功能组；图标大按钮、双行小按钮与段落样式库共用固定组高，组名在底部对齐。新建/保存可在开始标签提供快捷入口 |
| 模式与焦点上下文 | 不再单独常驻一行“文本 / 文档 › 名称”。数学等有实际操作意义的焦点上下文按阶段 2 在 Ribbon 按需展示 |
| 主编辑工作区 | 所见即所得为单一编辑面；源码模式为等宽双窗格，二者互斥 |
| 可选辅助面板 | 文件导航与诊断由“视图”功能组展开；诊断也可由状态栏展开；均默认不挤占正文 |
| 底部状态栏 | 当前模式、内容来源、排版后端/revision、保存状态、诊断数量、页码和缩放；保持一行优先 |

桌面起始尺寸：文档标签约 32、Ribbon 标签约 34、分组命令区约 104、状态栏约 30 逻辑像素。
功能区折叠后不保留命令区空位；Ctrl+F1、折叠按钮和双击标签均可收起，点击标签展开。
这些是初始布局参数，需要在原生字体、DPI、输入法与辅助功能环境下检验；不可牺牲文字或点击目标可用性。
辅助导航展开时建议约 166–220 逻辑像素，并允许调整；源码与预览继续平分余下宽度。
窄桌面窗口优先收起辅助面板，Ribbon 内容溢出时显示可拖动的横向滚动条；不同标签独立记忆滚动位置。
组名和命令标签不因窗口变窄而换行，主标签与折叠开关保持可见。会话草图低于 700 像素时上下排列源码与预览，仅用于窄展示区域阅读；
正式桌面应依据可读字号选择保留分屏或切换单窗格，不能默认把预览缩成缩略图。

## Ribbon 与文件菜单组织

- **文件**：新建、打开、最近打开、保存、另存为、导出、关闭。打印入口属于整体产品规划，单独确定交付阶段。
- **开始**：文档（新建/保存）、剪贴板、文字、段落样式、数学和历史；快捷键、菜单与功能区使用同一命令。
- **插入**：P1 的行内/独立公式、分数、根式、上下标与定界符；以后扩展图表、引用等文档对象。
- **布局**：纸张、页边距、方向、段落间距、页面显示及最终构建检查入口；能力按阶段开放。
- **视图**：工作方式、文件导航、诊断、缩放与分屏；改变显示不应修改文档内容。
- **帮助**：快捷键、命令说明与应用信息。

段落与标题角色、字符样式集中在“开始”，常见数学入口可同时出现在“开始”和“插入”。
未接入的能力置灰并按需提示原因；只读生成源码、组合输入等状态不能因切换 Ribbon 而绕过写入门禁。
剪贴板和格式命令须保留页面语义光标/选区，完成后继续在页面输入，不引入第二个正文编辑区。
...
- 新建、打开、保存、另存为、导出在文件菜单中可找到；常用操作可由 Ribbon 和键盘访问，未接入入口明确禁用。
- Ribbon 分组服务操作查找，可折叠以扩大正文；长文稿滚动不挤走必要操作，窄窗口可访问全部功能组。
- 点击粘贴、公式、段落样式后，输入继续落在原页面光标；切换标签或折叠不修改文档 revision。
...
所见即所得必须直接在排版后的正文与公式上放置光标、拖选和输入。
页面外的块输入框或覆盖整段的普通文本编辑器不满足此要求。命中坐标必须来自同一
文档、revision 与页码的编译字形；输入经语义会话提交，重排后光标跟随新几何。
