...
7. 混合构建 spike：正文/跨页表格、公式/图表、自定义宏、模板作用域、双向引用/最终页码/链接；
   矢量基线与注释桥接验证入口：[报告 0035](../spikes/SPK-0035-vector-fidelity.md)。
   分别以 LaTeX 和 Typst 为宿主输出，验证标准源码包与混合重建包。不能只做截图嵌入。
   六类内容各需成功与失败夹具，并区分源码支持、矢量载体与明确拒绝的组合；
   两端都做宿主和被嵌入方，验证真实矢量、引用反馈有界与失败不发布。
   原始夹具见[报告 0010](../spikes/SPK-0010-mixed-build.md)，标准包重建见
   [报告 0016](../spikes/SPK-0016-working-tree-status.md)。当前判定仅见[出口清单](../spikes/SPK-0012-exit-criteria.md)。
