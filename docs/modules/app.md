...
基础应用壳位于 `crates/scholium-app`：`chrome` 负责文档标签、工作方式切换与状态栏，
`ribbon` 负责文件菜单、功能标签与可折叠的分组命令区，`workspace` 负责辅助面板和分屏，
...
`ribbon::RibbonState` 只保存当前功能标签与折叠状态，不保存正文或修改 revision。
`ribbon/controls` 提供统一高度的分组、矢量图标大按钮与不换行的小按钮；`ribbon/groups` 编排
开始、插入、布局、视图的命令。窄窗口使用溢出时可见的水平滚动条，不把文字挤成多行或缩小点击区域。
Ctrl+F1 与收起按钮调用同一 `ToggleRibbon` 命令；点击标签展开，双击标签收起。

剪贴板按钮经原生 viewport 请求发送 Copy/Cut/Paste，先恢复页面焦点并保留原选区；
公式和文字入口复用页面编辑请求，段落样式发送 `SetKind`，均经会话提交，操作后继续在页面输入。
生成源码视图不启用这些正文写按钮；组合输入或已有待提交操作期间禁用相关写入。
新建、保存及工作方式切换仍受统一命令门禁约束。撤销/重做、结构数学、文档对象与纸张设置等
未接入能力只提供禁用入口，不以 Ribbon 布局表示能力交付。
...
[P1 排版工作区规划与参考图](../plan/P1_UI.md)：带文件菜单的 Ribbon、单一所见即所得编辑面、
...
未完成的公式使用编译器返回的当前 revision 字形，在页面原位置显示可编辑文字，
状态标记“公式待完成”；补全后恢复公式。此状态和无法生成页面的“排版失败”分别处理。
两种状态均有“查看原因”入口，诊断面板展示真实 Typst 消息并支持复制，不能保留占位诊断。
没有独立输入区域，不改写未完成公式的语义源串。

应用启动提前初始化排版 worker；结果到达主动唤醒界面，纹理采纳后继续请求一帧显示。
回到所见即所得时恢复页面输入焦点及已有光标。接口边界见
[ADR 0030](../adr/ADR-0030-incomplete-formula-feedback.md)。
