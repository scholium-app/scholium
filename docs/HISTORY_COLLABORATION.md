...
**引擎已选定（阶段 0）**：Loro 1.16.0（[ADR 0009](adr/0009-crdt-engine.md)、[报告 0014](spikes/0014-crdt-engines.md)）。依据是两个决定性能力只有它同时具备：**可移动树**（`LoroTree` 的 `mov`/`mov_to`/`mov_after`/`mov_before` 直接对应包裹/解除/重排）与**能过滤远端输入的内置 UndoManager**（实测撤销本地输入而保留远端输入）。Yrs 无 mov 类 API、Automerge 无 undo API。
