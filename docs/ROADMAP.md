...
7. 混合构建 spike：正文/跨页表格、公式/图表、自定义宏、模板作用域、双向引用/最终页码/链接；
   ...
   进度见[报告 0010](spikes/0010-mixed-build.md)：**结论 部分 Pass**——19 夹具 × 2 宿主 = **36/36 次运行通过**，
   ...
   **未验证**：标准目标包 / 可重建混合包的干净环境构建、行内矢量嵌入（仅块级）、
...
> 自动验证门禁的复核与修正后结果见[报告 0015](spikes/0015-verification-review.md)。
> **以上共 10 条。** 阶段 0 的逐条判定、证据与缺口见[报告 0012](spikes/0012-exit-criteria.md)。
> **判定结果：5 条满足、3 条部分满足、2 条不满足。** 两条不满足是第 3（预览时延 p95 实测 21ms/最坏 208ms）
> 与第 6（LaTeX 默认配置下能读项目外文件、TeX 自身配置挡不住）——都是**条件表述需要改写**加实现缺口，
> 不要把「改了措辞」当成「已经满足」。三条部分满足是第 1（跨节点选区删除）、第 8（行内嵌入与组件内部保真）、
> 第 10（干净环境构建标准包/可重建包未验证）。缺口清单与阶段 1 待办见报告 0012 文末。
