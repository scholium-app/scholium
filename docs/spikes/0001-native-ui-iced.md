...
**绘制框上沿**，而上下标位置又是用 `base_height - sub.height * 0.25` 这类临时常数凑的，
结果上标抬了一整行、下标降了一整行：

```text
修正前：底 "x" 基线 37.2 / 上标 "2" 基线 12.6（高 24.6px）/ 下标 "1" 基线 51.6（低 14.4px）
修正后：底 "x" 基线 33.7 / 上标 "2" 基线 25.3（高  8.4px = 0.42em）
                            / 下标 "1" 基线 37.7（低  4.0px = 0.20em）
```

改法是两条约定，写进 `core/src/layout.rs` 的模块说明：

1. **`Item::Text` 的纵坐标是基线**，不是上沿；渲染方必须用 `Item::top_of(baseline, size)` 换算，
   不能各自猜字体度量。`ASCENT_RATIO` 是唯一的近似常数，所有图元共用——同字号一定精确对齐，
   不同字号之间只剩与字号差成正比的小误差。
2. **上下标偏移改用排版惯例值**（上标 −0.42em、下标 +0.20em），不再用临时常数；
   `core/tests/layout.rs` 增加了对这两个偏移量的断言，防止再次漂移。

三个候选（iced / gpui / egui）的渲染方与截图已同步刷新。
核对数字用 `cargo run --example dump_layout`，不用对着截图猜。
