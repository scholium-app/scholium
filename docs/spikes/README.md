...
| 第 2 项 Typst 映射 | [0005-typst-mapping.md](0005-typst-mapping.md) | **结论 Pass（机制，含行内结构坐标）**：20 页 668 ms 编译；2050/2050 锚点解析；分数与上下标各有区间且反查 40/40 命中；增量编译 200 ms（冷的 30%）。已渲染首页叠加锚点核对（绿/红标记精确框住分数、上下标、矩阵、根式）。附带结论：预览必须异步去抖，不能按帧预算设计。 |
| 第 3–7 项 | 尚无 | 待执行 |
