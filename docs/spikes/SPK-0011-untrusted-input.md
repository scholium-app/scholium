...
- **本 spike 的 bwrap 配方还不完整**：恶意文档被成功阻断，但**良性文档在沙箱里编译失败**。
  ...
  干净的对照（同一份样例、同一组命名空间选项）：

  | 配方 | 纯拉丁文档 | 含中文文档 |
  |---|---|---|
  | 完整只读根 `--ro-bind / /` | **成功 3553 字节** | **成功 2098 字节** |
  | 受限挂载集（工具链 + `/etc/fonts` + `/etc/texmf` + `/var/lib/texmf` + `/etc/{ld.so.cache,passwd}` + 可写工作目录） | 失败 | 失败 |

  可见受限集的缺口**与字体无关**（纯拉丁文档同样失败）。已逐个排除的嫌疑：
  `/etc`（完整）、`/var`、`/run`、`/sys`、`/usr/local`、整个 `$HOME`、`~/.texlive`、
  `~/.cache/fontconfig`、`/etc/nsswitch.conf`、`/etc/localtime`，以及"工作目录只读"。
  **缺失依赖仍未定位。**

  还有一条必须在实现时注意：**不要把整个 `$HOME` 挂进沙箱**。实测在受限集里挂 `$HOME` 之后，
  `~/.ssh` 在沙箱内**可见**——这直接违背本项判据。真实实现只能挂必要的缓存子目录
  （或干脆用独立 HOME + 预生成的字体缓存）。
