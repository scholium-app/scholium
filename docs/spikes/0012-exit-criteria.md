...
- 对应：[路线图的 10 条出口条件](../ROADMAP.md#出口条件)。本次不放宽产品要求或修改已有 ADR。

## 当前判定

| # | 出口条件 | 判定 | 证据与剩余边界 |
|---|---|---|---|
| 1 | 原生候选共同验收、锁定版本与平台 | **部分满足** | egui 已选定；文本端点跨节点删除、纯文本复制/剪切、源码应用已有实现与 headless 测试。槽位端点仍拒绝，新路径未重跑桌面 IME/无障碍及完整共同验收 |
...
1. 完成条件 1 的真实窗口共同验收，明确或补齐槽位端点、结构选区及剪贴板范围。
