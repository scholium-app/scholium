...
### 与 Liii STEM 的对照（2026-09 调研）

[Liii STEM](https://liiistem.cn) 是 Mogan 社区版的商业版（TeXmacs 2.1.4 血统，
官方 FAQ 自述），叠加内置 AI、OCR、云备份。其全部文档（约 90 页）通读后的对照：

| Liii STEM 功能块 | 我们的计划 | 处置 |
|---|---|---|
| 结构化编辑、乐高符号、Tab 等价类、Alt+Shift+↓ 变体循环 | P2/输入手感阶段 | 已覆盖，是辨识度所在 |
| 焦点工具栏（随焦点变化的环境属性面板，其最核心交互设备） | **原计划没有** | **新增 P7，最小版随 P3 起逐步生长** |
| 文本格式（粗斜/颜色/高亮/删除线）、三种列表、脚注、引用块 | **原计划没有** | **新增 P3 文档结构** |
| 语义块（定理/引理/定义等约 20 种）、编号、加框 | **原计划没有** | **新增 P3，先做常用 8-10 种** |
| 图片/题注、表格/题注、代码块高亮 | **原计划没有** | **新增 P3** |
| 目录/图目录/表目录 | 没有 | 新增 P3（Typst `outline` 原生，便宜） |
| 标题环境（副标题/作者/日期）、摘要 | 没有 | 新增 P3 |
| 公式编号、label/reference 交叉引用、smart-ref | **原计划没有** | **新增 P6；smart-ref 不做** |
| 参考文献 .bib + GB/T 7714 国标样式 | 推在 1.0 后 | **提前到 P6 最小版**（hayagriva 原生支持国标） |
| 命令模式 `\` + Tab 补全 | 命令注册表有底，交互未定 | P4 明确补上 |
| Ctrl+Backspace 万能取消环境 | 没有 | **新增 P4**（`EditOp` 加 UnwrapNode，便宜且高频） |
| 结构化遍历（Alt+←/→ 逐结构移动、相似标签跳转） | 没有 | 新增 P4 |
| 结构化搜索替换（模式匹配、空白通配） | 1.0 后 | 维持 1.0 后 |
| 多行公式 align/eqnarray/aligned、cases | P5 有矩阵 | P5 已覆盖 |
| 连续滚动/单页/双页/全景视图 | 没有 | P7 最小版：连续滚动 + 单页 |
| 语义编辑全套（不可见乘号、CAS 互操作、语法红绿校验） | 没有 | 1.0 后（方向对，量太大） |
| OCR/魔法粘贴/AI 对话框/AI 自动排版 | §6 接口先行 | 1.0 后实装，边界不变 |
| 会话系统（Python/Maxima/Julia…）、TikZ/Gnuplot 插件 | 没有 | 1.0 后 |
| 交互式矢量绘图引擎、交换图 | 没有 | 1.0 后 |
| 幻灯片 beamer、折叠/图层 | 没有 | 1.0 后 |
| 源码模式（写类 LaTeX 源码） | — | **不做**（与结构化编辑定位冲突） |
| LaTeX/HTML/Markdown 导入导出、DOCX 导出、一键发布知乎 | 导入 1.0 后 | 维持；平台集成类不做 |
| 模板中心、样式深度自定义、宏编辑器、拼音注音、云备份 | 没有 | 1.0 后或不做 |

**结论：缺口在「文档广度」而非长尾。** P1 只有 4 种节点，P2-P7 之间没有
任何阶段把"写一篇真实文档"所需的结构补齐——这恰是 Liii STEM 文档里占比
最大的部分。据此新增 P3（文档结构），原 P3-P7 顺延为 P4-P8，MVP 累计
约 15-19 周。补阶段的纪律：只做 AST + 序列化 + 基本交互；样式自定义、
模板系统、平台集成一律 1.0 后。
...
### P3 — 文档结构（3-4 周）【2026-09 新增】

把"能写一篇真实文档"的结构补齐。Typst 对这些全有原生对应物，
序列化成本低；重点在 AST 设计与编辑交互。

- 行内格式：粗体/斜体/强调/等宽/删除线/高亮/颜色。
- 列表：无序/有序/描述，标记样式变体（为 Alt+Shift+↓ 循环提供对象）。
- 引用块、脚注。
- 语义块：定理/引理/推论/命题/定义/注记/示例/证明（8-10 种先行），
  自动编号、章节前缀、可选加框。
- 章节层级：章/节/子节（扩展 P1 的 heading）+ 自动编号 + 目录
  （Typst `outline`）、图目录/表目录。
- 图片（png/jpg/svg）+ 题注 figure；表格 + 题注；代码块 + 语法高亮。
- 标题环境（副标题/作者/日期）、摘要。

**验收**：能写一篇含定理环境、列表、脚注、图、表、代码块、目录的完整笔记，
全部结构可点击定位、可 undo。

### P4 — 输入手感（3-4 周）

命令注册表 + keymap（替代 Scheme 命令层）、**命令模式：`\` 进入 +
Tab 补全 + 回车插入**、乐高符号缓冲规则（`<`+`=` → `≤`）、
Tab 循环等价类（`a → α → ℵ`）、结构变体循环 Alt+Shift+↓（分数 普通/小型/
显示/斜线；定界符 无/圆/方/竖/花；列表标记；语义块类型；章节层级）、
**`Ctrl+Backspace` 取消环境（`EditOp::UnwrapNode`，一切结构通用）**、
**结构化遍历（Alt+←/→ 逐结构移动、Ctrl+PageUp/Down 相似标签跳转）**、
选区 + 结构键的结构化包裹、符号面板。
...
矩阵 / 分段函数（cases）/ 对齐公式的 AST 与编辑：单元格导航、
`Alt+方向` 插入行列、空单元格 Backspace 语义、align/aligned 多行环境
（自动对齐，无需手动 `&`）。
...
**编号与交叉引用**：公式/定理/图表的 label 与 reference（含悬停预览、
跳转）、编号开关与章节前缀。**参考文献最小版**：`.bib` 导入 +
`\cite` + GB/T 7714 国标样式（hayagriva 原生支持）。

### P7 — 应用外壳（3-4 周）

muda 菜单、rfd 对话框、文件树、大纲、偏好设置、accesskit 无障碍、
i18n（中/英）。**焦点工具栏**：随光标焦点显示当前环境的属性与操作
（编号开关、变体切换、定界符修改），结构化编辑器的核心交互设备，
从 P3 起以最小形态逐步生长，此处补全。**视图模式**：连续滚动（默认）
+ 单页切换。
...
AI provider 实装 + 建议态 UI（含 OCR/魔法粘贴、粘贴 LaTeX 片段转数学）、
结构化搜索替换（模式匹配/空白通配）、LaTeX/Markdown/`.tm` 导入导出、
文献管理增强、会话系统（Python/CAS）、矢量绘图、幻灯片（beamer）、
语义编辑全套（不可见运算符/CAS 互操作）、模板与样式深度自定义、拼音注音。
...
| 协作编辑 | CRDT 复杂度爆炸 |
| 1.0 内做格式转换 | 转换器是无底洞（1.0 后按需逐个做） |
| Typst 源码编辑模式 / 任何源码模式 | 与结构化编辑是两个产品 |
| 双渲染器 | 单引擎是本方案核心前提 |
| 1.0 内接真实 AI provider | 接口先行、实现后置（§6.4） |
| 1.0 内做会话/脚本执行、矢量绘图、幻灯片 | Liii STEM 的这些块各自都是一个产品的工作量 |
| 一键发布知乎等平台集成 | 平台 API 政策风险 + 非核心 |
| smart-ref 智能引用 | 引用文字模板化即可覆盖九成需求 |
...
| 又一次范围失控 | **高** | **高** | 硬性规定：P4 结束前不碰格式转换、AI provider 实现；文献仅限 P6 最小版 |
| 对照竞品后范围膨胀 | **高** | 高 | P3 只做 AST+序列化+基本交互；样式自定义、模板系统、会话、绘图、幻灯片全部 1.0 后（见 §7 对照表） |
